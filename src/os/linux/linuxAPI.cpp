...
void osAPI::attachSubSurfaceToWindow(windowId id, const subSurfaceSpec &subSurfaceToAttach)
{
	linuxWindowAPI::windowsInfo->getComponent(id)->subsurfaces[subSurfaceToAttach.subSurfaceSlot] =
		surface::allocateSurface(id, {surfaceRule::subsurface, subSurfaceToAttach.renderAPI, subSurfaceToAttach.width,
									  subSurfaceToAttach.height, subSurfaceToAttach.x, subSurfaceToAttach.y,
...
void osAPI::setsubSurfaceRenderEventListeners(windowId winId, int subSurfaceSlot,
											  void (*callback)(const cpuRenderData &))
{
