...
surfaceInfoComponent::surfaceInfoComponent(entityPool *pool): 
    pool(pool)
{
    pool->enlistType((void*)this, deleteCallback, &IdToIndex);
}

void surfaceInfoComponent::deleteComponent(entityId id)
{
...
surfaceData* surfaceInfoComponent::getComponent(entityId id)
{
    if(!pool->isIdValid(id) || IdToIndex[id.index] == -1)
        return nullptr;
...
void surfaceInfoComponent::setComponent(entityId id, surfaceData buffer)
{
