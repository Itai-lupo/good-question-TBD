...
mouseMoveCallbackComponent::mouseMoveCallbackComponent(entityPool *pool): 
    pool(pool)
{
    pool->enlistType((void*)this, deleteCallback, &IdToIndex);
}

void mouseMoveCallbackComponent::deleteComponent(entityId id)
{
...
mouseMoveCallback mouseMoveCallbackComponent::getCallback(entityId id)
{
    if(!pool->isIdValid(id) || IdToIndex[id.index] == -1)
        return nullptr;
...
void mouseMoveCallbackComponent::setCallback(entityId id, mouseMoveCallback callback)
{
...
mouseMoveCallbackComponent::~mouseMoveCallbackComponent()
{
