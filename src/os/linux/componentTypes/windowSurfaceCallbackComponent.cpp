...
windowSurfaceCallbackComponent::windowSurfaceCallbackComponent(entityPool *pool): 
    pool(pool)
{
    pool->enlistType((void*)this, deleteCallback, &IdToIndex);
}

void windowSurfaceCallbackComponent::deleteComponent(entityId id)
{
...
surfaceCallback windowSurfaceCallbackComponent::getCallback(entityId id)
{
    if(!pool->isIdValid(id) || IdToIndex[id.index] == -1)
        return nullptr;
...
void windowSurfaceCallbackComponent::setCallback(entityId id, surfaceCallback callback)
{
