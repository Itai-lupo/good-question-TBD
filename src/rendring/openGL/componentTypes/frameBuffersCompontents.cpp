...
frameBuffersCompontents::frameBuffersCompontents(entityPool *pool): 
    pool(pool)
{
    pool->enlistType((void*)this, deleteCallback, &IdToIndex);
}

void frameBuffersCompontents::deleteComponent(entityId id)
{
...
frameBufferInfo* frameBuffersCompontents::getComponent(entityId id)
{
    if(!pool->isIdValid(id) || IdToIndex[id.index] == -1)
        return nullptr;
...
void frameBuffersCompontents::setComponent(entityId id, frameBufferInfo buffer)
{
...
frameBuffersCompontents::~frameBuffersCompontents()
{
