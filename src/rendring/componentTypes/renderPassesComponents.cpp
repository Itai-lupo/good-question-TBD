...
renderPassesComponents::renderPassesComponents(entityPool *pool): 
    pool(pool)
{
    pool->enlistType((void*)this, deleteCallback, &IdToIndex);
}

void renderPassesComponents::deleteComponent(entityId id)
{
...
renderPassInfo *renderPassesComponents::getComponent(entityId id)
{
    if(!pool->isIdValid(id) || IdToIndex[id.index] == -1)
        return nullptr;
...
void renderPassesComponents::setComponent(entityId id, renderPassInfo& buffer)
{
...
renderPassesComponents::~renderPassesComponents()
{
