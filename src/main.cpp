...
float *vertices3 = new float[]{
	0.5f, 0.5f, 0.0f, 1.0f, 1.0f, 0.5f, -0.5f, 0.0f, 1.0f, 0, -0.5f, -0.5f, 0.0f, 0, 0, -0.5f, 0.5f, 0.0f, 0.0f, 1.0f,
};
...
float *vertices2 = new float[]{
	1.0f, 1.0f, 1.0f, 0, 0, 0, 0.0f, 1.0f,
};
...
{
	LOG_INFO(sendor.id.index << ", " << sendor.utf8Buffer << ", " << keyCodeToString(sendor.value) << "("
							 << (int)sendor.value << ")");
}
...
{
	LOG_INFO("pointer axis " << mouseAxisToString(sendor.axis) << "from source "
							 << mouseAxisSourceToString(sendor.axisSource) << " = " << sendor.discreteValue << " or "
							 << sendor.value);
}
...
{
	{
		...
		if (!temp)
		{
			...
			sendor.api->setShader(
				{shaderToRender, "assets/shaders/vertex/texture.vert.spv", "assets/shaders/fragment/texture.frag.spv"});
			...
			tempTex.bufferToLoad =
				new loadTextureRequst{0, 0, (uint32_t)width, (uint32_t)height, textureFormat::RGB8, 0x1401, data};
		...
		sendor.api->renderRequest({
			...
			.drawCalls = {{.shader = shaderToRender,
						   .vertexArrayId = shapeToRender2,
						   .texturesIds = {[0] = testTexture, [1 ... 31] = {16777215, 255}},
						   .UBO = UBO,
